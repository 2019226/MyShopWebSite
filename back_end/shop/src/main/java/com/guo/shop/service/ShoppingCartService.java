...
public class ShoppingCartService {
    ...
    public String getTotalFromShoppingCart(String userId){
        ...
        Double tmp = (double) dataBaseService.query(queryTotalPriceSql,userId).get(0).get("total");
    ...
    public boolean checkOutHandle(String userId) {
        String sql = "select product_id, shop_product.name, shop_product.quantity , shopping_cart.demand_quantity, if(cast(shop_product.quantity as SIGNED) >= cast(shopping_cart.demand_quantity as SIGNED),'pass','failed') as 'validate' from shopping_cart,shop_product where id = product_id and user_id=?";
